...
def classify_asset(symbol: str) -> str:
    ...
    if s.endswith(("USDT","USDC","BTC","ETH")) and len(s) >= 6: return "crypto"
...
def fetch_binance(symbol: str, interval="1h", limit=500):
...
def fetch_binance_quote(symbol: str):
    sym = _binance_symbol(symbol)
    r=requests.get(f"{BINANCE}/ticker/24hr", params={"symbol":sym}, timeout=10); r.raise_for_status(); q=r.json()
    return {"price":float(q["lastPrice"]),"change_pct":float(q["priceChangePercent"]),"source":"Binance","freshness":"LIVE / exchange API"}

def fetch_alpha_vantage(symbol: str, interval="1h", asset_type="stock"):
    key=_secret("ALPHA_VANTAGE_API_KEY")
    if not key: raise RuntimeError("ALPHA_VANTAGE_API_KEY is not configured")
    if asset_type=="forex":
        s=symbol.upper().replace("/","").replace("-","")
        if len(s)!=6: raise ValueError("Forex symbol must look like EURUSD or EUR/USD")
        params={"function":"FX_INTRADAY","from_symbol":s[:3],"to_symbol":s[3:],"interval":{"1h":"60min","4h":"60min"}.get(interval,interval),"apikey":key,"outputsize":"compact"}
    else:
        params={"function":"TIME_SERIES_INTRADAY","symbol":symbol.upper(),"interval":{"1h":"60min","4h":"60min"}.get(interval,interval),"outputsize":"compact","apikey":key}
    r=requests.get(AV,params=params,timeout=20); r.raise_for_status(); data=r.json()
    ...
    return df[["Open","High","Low","Close","Volume"]].dropna(), {"source":"Alpha Vantage","freshness":"Provider entitlement dependent","asset_type":asset_type}
...
def fetch_history(symbol, period="6mo", interval="1h", provider="auto"):
    ...
    if provider=="auto" and asset=="crypto":
        try:
            df,meta=fetch_binance(symbol,interval); return _binance_symbol(symbol),df,meta
        except Exception:
            pass
    if provider in ("auto","alpha") and asset in ("stock","forex") and _secret("ALPHA_VANTAGE_API_KEY"):
        try:
            df,meta=fetch_alpha_vantage(symbol,interval,asset); return normalize_symbol(symbol),df,meta
